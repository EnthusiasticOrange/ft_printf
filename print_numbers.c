...
/*   Updated: 2020/07/28 19:18:37 by mjuli            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_padding(t_arg_info *arg, char padding)
{
	...
	while (arg->width-- > 0)
		arg->result += write(1, &padding, 1);
}

static void	print_number(t_arg_info *arg, char buf[SIZE], int digits)
{
	arg->width -= digits > arg->precision ? digits : arg->precision;
	if (!check_flag(arg, FLAG_DASH) && (!check_flag(arg, FLAG_ZERO) ||
		arg->precision != -1))
		print_padding(arg, ' ');
	if (arg->print_sign)
		arg->result += write(1, &arg->print_sign, 1);
	if (arg->prefix)
		arg->result += write(1, arg->prefix, ft_strlen(arg->prefix));
	if (!check_flag(arg, FLAG_DASH) && check_flag(arg, FLAG_ZERO) &&
		arg->precision == -1)
		print_padding(arg, '0');
	while (arg->precision-- > digits)
		arg->result += write(1, "0", 1);
	arg->result += write(1, buf + SIZE - digits, digits);
