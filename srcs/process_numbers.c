...
/*   Updated: 2020/07/28 19:16:50 by mjuli            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static unsigned long long	process_decimal(t_arg_info *arg)
{
	...
	if (d >= 0)
	{
		if (check_flag(arg, FLAG_PLUS))
			arg->print_sign = '+';
		else if (check_flag(arg, FLAG_SPACE))
			arg->print_sign = ' ';
		u = d;
	}
	else
...
static unsigned long long	process_hex(t_arg_info *arg)
{
	...
	if (check_flag(arg, FLAG_SHARP) && u != 0)
